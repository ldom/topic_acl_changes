...
def is_ignored(line):
    return line.startswith(TIMESTAMP_START) or len(line.strip()) == 0


def is_main_line(line):
    return line.startswith(MAIN_START)
...
def parse_main_line(line):
    ...
    start = line.find(RESOURCE_PATTERN_START)
    if start == -1:
        raise RuntimeError("Missing 'ResourcePattern()' in resource header")

    resource_ptn_str = line[start + len(RESOURCE_PATTERN_START):len(line) - 3]
...
def parse_acl_line(line):
    ...
    return {
        'acl_principal': acl_principal[1],
        'acl_host': acl_host[1],
        'acl_operation': acl_operation[1],
        'acl_permission_type': acl_permission_type[1]
    }


def parse_acl_output(acl_output) -> List[Dict]:
    ...
    for line in lines:
        ...
        if is_main_line(line):
            ...
            resource_type, resource_name, pattern_type = parse_main_line(line)
            acls = []
        else:
